...
class RegionDetailsFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        viewModel = ViewModelProviders.of(this, newViewModelFactory { RegionDetailsViewModel(context!!, tid) })
                .get(RegionDetailsViewModel::class.java)
        return render().view
    }
    ...
    private fun render() = UI {
        verticalLayout {
            linearLayout {
                ...
                textView {
                    viewModel.bind { text = viewModel.timeFrom.getValue("-") + " 至 " + viewModel.timeTo.getValue("-") }
                    textSize = 14f
                    setOnClickListener {
...
//                    IntentHandlerUtil.openWithPlayer(activity!!, IntentHandlerUtil.TYPE_VIDEO, item.id)
                ...
                loadMoreView {
                    ...
                    viewModel.bind(viewModel::loadState) {
                        DebugMiao.log("viewModel.loadState = " + viewModel.loadState)
                        state = it
                    }
                }
            }.onLoadMore {
