...
import cn.a10miaomiao.bilimiao.compose.common.platform.getBrowserCookie
...
private class TestPageViewModel(
    ...
    val cookie: String
        get() = getBrowserCookie("https://passport.bilibili.com/")

    val buvid: String
        get() = BilimiaoCommCore.instance.getBilibiliBuvid()

    val loginInfoText: String
        get() = BilimiaoCommCore.instance.loginInfo?.toString() ?: ""
...
private fun TestPageContent(
    ...
    Column(
        ...
        Button(onClick = {
            showLoginInfo.value = !showLoginInfo.value
        }) {
            if (showLoginInfo.value) {
                Text(text = "隐藏")
            } else {
                Text(text = "显示")
            }
